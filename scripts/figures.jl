#!/usr/bin/env julia
# Generate all figures for SlenderConeRecoil.
# Usage: julia --project=scripts scripts/figures.jl [--metadata-only]

using Dates
using TOML
using Plots
using SlenderConeRecoil
default(dpi=200, size=(700, 450), linewidth=2, legend=:topright,
        guidefontsize=12, tickfontsize=10, legendfontsize=9)

const PROJECT_ROOT = normpath(joinpath(@__DIR__, ".."))
const FIGDIR = joinpath(PROJECT_ROOT, "figures")
const FIGURE_FORMATS = ("pdf", "png")
const FIGURE_STEMS = (
    "fig1_similarity_profile",
    "fig2_blob_excess",
    "fig3_recoil_profiles",
    "fig4_tip_shape",
    "fig5_velocity",
    "fig6_matched_asymptotic",
    "fig6_matched_excess",
)
const FIGURE_EPSILON = 0.1
const INNER_XI0_INITIAL = 2.79
const INNER_S0_INITIAL = 0.28
const INNER_SXIXX0_INITIAL = 0.57
const INNER_XI_MAX = 60.0
const INNER_NEWTON_ITERS = 30
const INNER_NEWTON_TOL = 1e-4
const INNER_ODE_MAXITERS = 2_000_000
const FIGURE6_XI_MATCH = 15.0
const FIGURE6_XI_MAX = 80.0
const FIGURE6_BLEND_HALF_WIDTH = 2.0
const OUTER_LINEAR_MAXITERS = 500_000
const OUTER_FULL_MAXITERS = 1_000_000
const REGENERATION_COMMAND = "julia --project=scripts scripts/figures.jl"
mkpath(FIGDIR)

function save_figure(p, stem::AbstractString)
    saved = String[]
    for ext in FIGURE_FORMATS
        path = joinpath(FIGDIR, "$stem.$ext")
        savefig(p, path)
        push!(saved, relpath(path, PROJECT_ROOT))
    end
    println("  Saved: ", join(saved, ", "))
    saved
end

function _toml_string(s::AbstractString)
    escaped = replace(String(s), "\\" => "\\\\", "\"" => "\\\"", "\n" => "\\n")
    "\"$escaped\""
end

_toml_value(x::AbstractString) = _toml_string(x)
_toml_value(x::Symbol) = _toml_string(string(x))
_toml_value(::Nothing) = _toml_string("nothing")
_toml_value(x::Bool) = string(x)
_toml_value(x::Integer) = string(x)
_toml_value(x::AbstractFloat) = isfinite(x) ? string(x) : _toml_string(string(x))
_toml_value(x::AbstractVector) = "[" * join((_toml_value(v) for v in x), ", ") * "]"
_toml_value(x) = _toml_string(string(x))

function _write_kv(io, key::AbstractString, value)
    println(io, key, " = ", _toml_value(value))
end

function _read_command(args::Vector{String})
    try
        strip(read(Cmd(args), String))
    catch
        ""
    end
end

function _git_commit()
    commit = _read_command(["git", "-C", PROJECT_ROOT, "rev-parse", "--short", "HEAD"])
    isempty(commit) ? "unknown" : commit
end

function _git_dirty()
    status = _read_command(["git", "-C", PROJECT_ROOT, "status", "--porcelain"])
    !isempty(status)
end

function _project_version()
    project = TOML.parsefile(joinpath(PROJECT_ROOT, "Project.toml"))
    string(get(project, "version", "unknown"))
end

function _write_diagnostics(io, table::AbstractString, diagnostics)
    diagnostics === nothing && return
    println(io)
    println(io, "[$table]")
    fields = diagnostics isa DiagnosticSummary ? as_namedtuple(diagnostics) :
             diagnostics isa NamedTuple ? diagnostics :
             as_namedtuple(diagnostic_summary(diagnostics))
    for key in keys(fields)
        value = getfield(fields, key)
        if !(value isa NamedTuple) && !(value isa Tuple)
            _write_kv(io, string(key), value)
        end
    end
end

function _write_wave_diagnostics(io, table::AbstractString, diagnostics)
    diagnostics === nothing && return
    println(io)
    println(io, "[$table]")
    _write_kv(io, "status", diagnostics.status)
    _write_kv(io, "successful", diagnostics.successful)
    _write_kv(io, "profile_kind", diagnostics.profile_kind)
    _write_kv(io, "epsilon", diagnostics.epsilon)
    _write_kv(io, "epsilon_source", diagnostics.epsilon_source)
    _write_kv(io, "source_status", diagnostics.source_status)
    _write_kv(io, "quantitative_source_status",
              diagnostics.quantitative_source_status)
    _write_kv(io, "qualitative_source_id",
              diagnostics.source_basis.qualitative.source_id)
    _write_kv(io, "qualitative_source_status",
              diagnostics.source_basis.qualitative.source_status)

    _write_kv(io, "sample_count", diagnostics.samples.count)
    _write_kv(io, "xi_min", diagnostics.samples.xi_min)
    _write_kv(io, "xi_max", diagnostics.samples.xi_max)
    _write_kv(io, "max_abs_excess", diagnostics.samples.max_abs_excess)
    _write_kv(io, "deadband", diagnostics.samples.deadband)

    _write_kv(io, "zero_crossing_count", diagnostics.zero_crossings.count)
    _write_kv(io, "zero_crossing_xi", diagnostics.zero_crossings.xi)
    _write_kv(io, "crest_count", diagnostics.extrema.crest_count)
    _write_kv(io, "crest_xi", diagnostics.extrema.crests.xi)
    _write_kv(io, "crest_value", diagnostics.extrema.crests.value)
    _write_kv(io, "trough_count", diagnostics.extrema.trough_count)
    _write_kv(io, "trough_xi", diagnostics.extrema.troughs.xi)
    _write_kv(io, "trough_value", diagnostics.extrema.troughs.value)

    _write_kv(io, "local_wavelength_xi",
              diagnostics.wavelength.zero_crossing.xi)
    _write_kv(io, "local_wavelength",
              diagnostics.wavelength.zero_crossing.value)
    _write_kv(io, "median_wavelength",
              diagnostics.wavelength.zero_crossing.median)
    _write_kv(io, "phase_zero_crossing_xi",
              diagnostics.phase.zero_crossing_xi)
    _write_kv(io, "phase_zero_crossing_value",
              diagnostics.phase.zero_crossing_phase)
    _write_kv(io, "phase_local_xi", diagnostics.phase.local_phase.xi)
    _write_kv(io, "phase_local_value", diagnostics.phase.local_phase.phase)

    _write_kv(io, "envelope_status", diagnostics.envelope.status)
    _write_kv(io, "envelope_points", diagnostics.envelope.points)
    _write_kv(io, "envelope_xi", diagnostics.envelope.xi)
    _write_kv(io, "envelope_amplitude", diagnostics.envelope.amplitude)
    _write_kv(io, "envelope_decay_rate", diagnostics.envelope.decay_rate)
    _write_kv(io, "envelope_decay_length", diagnostics.envelope.decay_length)
    _write_kv(io, "envelope_fit_rms", diagnostics.envelope.fit_rms)
    _write_kv(io, "envelope_amplitude_ratio",
              diagnostics.envelope.amplitude_ratio)

    _write_kv(io, "resolution_status", diagnostics.resolution.status)
    _write_kv(io, "min_samples_per_wavelength",
              diagnostics.resolution.min_samples_per_wavelength)
    _write_kv(io, "median_samples_per_wavelength",
              diagnostics.resolution.median_samples_per_wavelength)
    _write_kv(io, "resolution_threshold", diagnostics.resolution.threshold)

    _write_kv(io, "qualitative_oscillatory",
              diagnostics.qualitative.oscillatory)
    _write_kv(io, "qualitative_modulated_on_longer_scale",
              diagnostics.qualitative.modulated_on_longer_scale)
    _write_kv(io, "qualitative_source_consistency",
              diagnostics.qualitative.source_consistency)
    _write_kv(io, "modulation_scale_ratio",
              diagnostics.qualitative.modulation_scale_ratio)
end

function _require_successful_figure_diagnostics(label::AbstractString, diagnostics)
    diagnostics === nothing && return
    diagnostics_succeeded(diagnostics) ||
        error("$label diagnostics failed; refusing to write figure metadata")
end

function write_figure_metadata(; sol=nothing, outer_linearised=nothing,
                               outer_full=nothing, pde=nothing)
    path = joinpath(FIGDIR, "metadata.toml")
    generated_at = Dates.format(Dates.now(), dateformat"yyyy-mm-ddTHH:MM:SS")
    package_version = _project_version()
    git_commit = _git_commit()
    git_dirty = _git_dirty()
    _require_successful_figure_diagnostics("inner solution", sol)
    _require_successful_figure_diagnostics("outer linearised", outer_linearised)
    _require_successful_figure_diagnostics("outer full", outer_full)
    _require_successful_figure_diagnostics("PDE solution", pde)
    inner_wave = sol === nothing ? nothing :
                 wave_diagnostics(sol; epsilon=FIGURE_EPSILON)
    open(path, "w") do io
        println(io, "# Figure regeneration metadata for SlenderConeRecoil.jl.")
        println(io, "# Rewritten by scripts/figures.jl; tracked figures are not regenerated by tests.")
        _write_kv(io, "generated_at_local", generated_at)
        _write_kv(io, "regeneration_command", REGENERATION_COMMAND)
        _write_kv(io, "figure_formats", collect(FIGURE_FORMATS))
        _write_kv(io, "figure_stems", collect(FIGURE_STEMS))

        println(io)
        println(io, "[environment]")
        _write_kv(io, "julia_version", string(VERSION))
        _write_kv(io, "package_name", "SlenderConeRecoil")
        _write_kv(io, "package_version", package_version)
        _write_kv(io, "git_commit", git_commit)
        _write_kv(io, "git_dirty", git_dirty)

        println(io)
        println(io, "[parameters]")
        _write_kv(io, "epsilon", FIGURE_EPSILON)
        _write_kv(io, "inner_xi0_initial", INNER_XI0_INITIAL)
        _write_kv(io, "inner_S0_initial", INNER_S0_INITIAL)
        _write_kv(io, "inner_Sxixi0_initial", INNER_SXIXX0_INITIAL)
        _write_kv(io, "inner_xi_max", INNER_XI_MAX)
        _write_kv(io, "inner_newton_iters", INNER_NEWTON_ITERS)
        _write_kv(io, "inner_newton_tol", INNER_NEWTON_TOL)
        _write_kv(io, "inner_ode_maxiters", INNER_ODE_MAXITERS)
        _write_kv(io, "figure6_xi_match", FIGURE6_XI_MATCH)
        _write_kv(io, "figure6_xi_max", FIGURE6_XI_MAX)
        _write_kv(io, "figure6_blend_half_width", FIGURE6_BLEND_HALF_WIDTH)
        _write_kv(io, "outer_linearised_maxiters", OUTER_LINEAR_MAXITERS)
        _write_kv(io, "outer_full_maxiters", OUTER_FULL_MAXITERS)

        if sol !== nothing
            println(io)
            println(io, "[inner_solution]")
            _write_kv(io, "converged", sol.converged)
            _write_kv(io, "termination_reason", sol.termination_reason)
            _write_kv(io, "iterations", sol.iterations)
            _write_kv(io, "final_residual_norm", sol.final_residual_norm)
            _write_kv(io, "xi0", sol.ξ₀)
            _write_kv(io, "S0", sol.S₀)
            _write_kv(io, "Sxixi0", sol.Sξξ₀)

            _write_diagnostics(io, "inner_solution_diagnostics",
                               diagnostic_summary(sol))
            _write_wave_diagnostics(io, "inner_wave_diagnostics",
                                    inner_wave)
        end

        _write_diagnostics(io, "outer_linearised_diagnostics", outer_linearised)
        _write_diagnostics(io, "outer_full_diagnostics", outer_full)
        _write_diagnostics(io, "pde_solution_diagnostics", pde)
    end
    println("Metadata saved: ", relpath(path, PROJECT_ROOT))
    path
end

# ── Figure 1: S(ξ) near tip showing the blob ──────────────────────────
function figure1(sol)
    println("Figure 1: Similarity profile S(ξ) near tip...")
    ε = FIGURE_EPSILON

    # Zoomed view near tip where the blob is visible
    p = plot(xlabel="ξ", ylabel="S(ξ)",
             title="Similarity profile near tip (ε=$ε)")

    idx = sol.ξ .< 15
    plot!(p, sol.ξ[idx], sol.S[idx], label="S(ξ)", color=:blue, linewidth=2.5)

    # Undisturbed cone (only for ξ > 0)
    ξ_ref = range(0, 15, length=200)
    plot!(p, ξ_ref, ε .* ξ_ref, label="εξ (undisturbed)", color=:red,
          linestyle=:dash, linewidth=1.5)

    scatter!(p, [sol.ξ₀], [sol.S₀], label="tip: ξ₀=$(round(sol.ξ₀,digits=2)), S₀=$(round(sol.S₀,digits=3))",
             color=:red, markersize=6)

    # Annotate the blob excess
    annotate!(p, sol.ξ₀ + 1.5, sol.S₀ + 0.08,
              text("blob excess\n≈ $(round(sol.S₀ - ε*sol.ξ₀, digits=3))", 8, :left))

    xlims!(p, 0, 15)
    ylims!(p, 0, 1.8)

    save_figure(p, "fig1_similarity_profile")
end

# ── Figure 2: Excess profile S(ξ) - εξ showing blob structure ─────────
function figure2(sol)
    println("Figure 2: Excess profile S - εξ (blob structure)...")
    ε = FIGURE_EPSILON

    excess = sol.S .- ε .* sol.ξ

    p = plot(xlabel="ξ", ylabel="S(ξ) - εξ",
             title="Blob excess above undisturbed cone (ε=$ε)")

    plot!(p, sol.ξ, excess, label="S(ξ) - εξ", color=:blue, linewidth=2.5)
    hline!(p, [0], label="", color=:gray, linestyle=:dash, alpha=0.5)
    vline!(p, [sol.ξ₀], label="tip ξ₀", color=:red, linestyle=:dot)

    # The excess starts at S₀ - εξ₀ and decays to 0
    scatter!(p, [sol.ξ₀], [sol.S₀ - ε*sol.ξ₀], label="",
             color=:red, markersize=5)

    xlims!(p, sol.ξ₀ - 0.5, 40)
    ylims!(p, -0.005, maximum(excess)*1.2)

    save_figure(p, "fig2_blob_excess")
end

# ── Figure 3: Physical recoil — cone profile at multiple times ─────────
function figure3(sol)
    println("Figure 3: Physical cone recoil at multiple times...")
    ε = FIGURE_EPSILON

    p = plot(xlabel="z", ylabel="R(z,t)",
             title="Cone tip recoil: profile at different times (ε=$ε)")

    times = [0.01, 0.05, 0.2, 0.5, 1.0, 2.0]
    colors = cgrad(:viridis, length(times), categorical=true)

    for (k, t) in enumerate(times)
        ℓ = t^(2/3)
        z_vals = sol.ξ .* ℓ
        R_vals = sol.S .* ℓ
        z_tip = sol.ξ₀ * ℓ
        R_tip = sol.S₀ * ℓ

        # Add hemispherical cap at tip (upper half: axis → profile)
        z_cap, r_cap = tip_cap_upper(z_tip, R_tip)
        z_full = vcat(z_cap, z_vals)
        R_full = vcat(r_cap, R_vals)

        idx = z_full .< 10
        plot!(p, z_full[idx], R_full[idx], label="t=$t", color=colors[k])
    end

    # Undisturbed cone for reference
    z_ref = range(0, 10, length=100)
    plot!(p, z_ref, ε .* z_ref, label="R=εz", color=:black, linestyle=:dash,
          linewidth=1, alpha=0.5)

    # Mark tip positions at each time
    for (k, t) in enumerate(times)
        ℓ = t^(2/3)
        z_tip = sol.ξ₀ * ℓ
        R_tip = sol.S₀ * ℓ
        if z_tip < 10
            scatter!(p, [z_tip], [R_tip], label="", color=colors[k],
                     markersize=4, markerstrokewidth=0)
        end
    end

    xlims!(p, 0, 10)
    ylims!(p, 0, 1.2)

    save_figure(p, "fig3_recoil_profiles")
end

# ── Helper: hemispherical cap at the tip ────────────────────────────────
"""
Left-facing semicircular cap connecting 1D profile (R=R_tip at z=z_tip)
to R=0 on the axis. The cap curves LEFT from z_tip to z_tip - R_tip.

Full cap (upper+lower): θ from π/2 → 0 → -π/2
  z = z_tip - R_tip·cos(θ),  r = R_tip·sin(θ)

Upper half only: θ from 0 → π/2 (r goes 0 → R_tip, z goes z_tip-R_tip → z_tip)
"""
function tip_cap(z_tip, R_tip; n=50)
    # Full cap: from top (r=+R_tip) around left to bottom (r=-R_tip)
    θ = range(π/2, -π/2, length=n)
    z_cap = z_tip .- R_tip .* cos.(θ)
    r_cap = R_tip .* sin.(θ)
    (z_cap, r_cap)
end

function tip_cap_upper(z_tip, R_tip; n=30)
    # Upper half only: from axis (r=0) to profile start (r=R_tip)
    θ = range(0, π/2, length=n)
    z_cap = z_tip .- R_tip .* cos.(θ)
    r_cap = R_tip .* sin.(θ)
    (z_cap, r_cap)
end

# ── Figure 4: 2D axisymmetric shape — zoomed on tip ───────────────────
function figure4(sol)
    println("Figure 4: Axisymmetric tip shape (zoomed)...")
    ε = FIGURE_EPSILON
    t = 1.0
    ℓ = t^(2/3)
    z_vals = sol.ξ .* ℓ
    R_vals = sol.S .* ℓ

    z_tip = sol.ξ₀ * ℓ
    R_tip = sol.S₀ * ℓ

    # Zoom tightly on tip: from just before cap to a few R_tip beyond
    z_lo = z_tip - 2*R_tip
    z_hi = z_tip + 12*R_tip
    idx = (z_vals .>= z_tip) .& (z_vals .< z_hi)

    # Build closed profile: cap + upper + lower (reversed)
    z_cap, r_cap = tip_cap(z_tip, R_tip, n=80)
    z_upper = z_vals[idx]
    r_upper = R_vals[idx]
    z_lower = reverse(z_upper)
    r_lower = -reverse(r_upper)

    z_outline = vcat(z_cap, z_upper, z_lower)
    r_outline = vcat(r_cap, r_upper, r_lower)

    p = plot(xlabel="z", ylabel="r",
             title="Recoiled tip shape (t=$t, ε=$ε)",
             aspect_ratio=:equal, size=(700, 500))

    # Filled fluid body
    plot!(p, z_outline, r_outline, seriestype=:shape,
          label="fluid body", fillcolor=:blue, alpha=0.3,
          linecolor=:blue, linewidth=2)

    # Undisturbed cone
    z_cone = range(z_lo, z_hi, length=100)
    plot!(p, z_cone, ε .* z_cone, label="undisturbed cone",
          color=:red, linestyle=:dash, linewidth=1.5)
    plot!(p, z_cone, -ε .* z_cone, label="", color=:red, linestyle=:dash,
          linewidth=1.5)

    xlims!(p, z_lo, z_hi)
    R_max = maximum(r_upper) * 1.5
    ylims!(p, -R_max, R_max)

    save_figure(p, "fig4_tip_shape")
end

# ── Figure 5: Velocity profile ─────────────────────────────────────────
function figure5(sol)
    println("Figure 5: Velocity profile...")
    ε = FIGURE_EPSILON

    p = plot(xlabel="ξ", ylabel="U(ξ)",
             title="Similarity velocity (ε=$ε): fluid pushed away from tip")
    plot!(p, sol.ξ, sol.U, label="U(ξ)", color=:blue, linewidth=2.5)
    hline!(p, [0], label="", color=:gray, linestyle=:dash, alpha=0.3)
    vline!(p, [sol.ξ₀], label="tip ξ₀", color=:red, linestyle=:dot)

    # U > 0 near tip (fluid moves in +ξ direction = recoil)
    # U → 0 far from tip
    xlims!(p, sol.ξ₀ - 0.5, 30)

    save_figure(p, "fig5_velocity")
end

# ── Figure 6: Matched asymptotic — tip to body ──────────────────────────
function figure6(inner_sol)
    println("Figure 6: Matched asymptotic (tip to body)...")
    ε = FIGURE_EPSILON; ξ₀ = inner_sol.ξ₀; S₀ = inner_sol.S₀
    ξ_match = FIGURE6_XI_MATCH

    # Local CAS algebra check: integer/Laurent ε grammar, not source hierarchy.
    println("  CAS: deriving local outer hierarchy...")
    eqs = derive_outer_equations(order=5)
    println("  Orders with nonzero terms: ", sort(collect(keys(eqs))))

    # Solve both: linearised and current reconstructed nonlinear outer systems.
    println("  Solving linearised outer...")
    outer_lin = solve_outer_linearised(inner_sol, ξ_match=ξ_match,
                                       ξ_max=FIGURE6_XI_MAX,
                                       maxiters=OUTER_LINEAR_MAXITERS)
    _require_successful_figure_diagnostics("outer linearised", outer_lin)
    println("  Solving full nonlinear outer...")
    outer_full = solve_outer_full(inner_sol, ξ_match=ξ_match,
                                  ξ_max=FIGURE6_XI_MAX,
                                  maxiters=OUTER_FULL_MAXITERS)
    _require_successful_figure_diagnostics("outer full", outer_full)

    # Build composite: inner for ξ < ξ_match, full outer for ξ > ξ_match
    ξ_all = range(ξ₀, FIGURE6_XI_MAX, length=1000) |> collect
    S_comp = zeros(length(ξ_all))
    blend_lo = ξ_match - FIGURE6_BLEND_HALF_WIDTH
    blend_hi = ξ_match + FIGURE6_BLEND_HALF_WIDTH

    for (i, ξ) in enumerate(ξ_all)
        Si = SlenderConeRecoil._interp_scalar(inner_sol.ξ, inner_sol.S, ξ)
        So = ξ < outer_full.ξ[1] ? ε * ξ :
             SlenderConeRecoil._interp_scalar(outer_full.ξ, outer_full.S, ξ)
        if ξ ≤ blend_lo;      S_comp[i] = Si
        elseif ξ ≥ blend_hi;  S_comp[i] = So
        else w = (ξ - blend_lo)/(blend_hi - blend_lo)
             S_comp[i] = (1-w)*Si + w*So
        end
    end

    # Hemispherical cap
    θ_cap = range(0, π/2, length=50)
    ξ_cap = ξ₀ .- S₀ .* cos.(θ_cap)
    S_cap = S₀ .* sin.(θ_cap)
    ξ_full = vcat(ξ_cap, ξ_all)
    S_full = vcat(S_cap, S_comp)

    # ── Main plot: S(ξ) tip to body ──
    p = plot(xlabel="ξ", ylabel="S(ξ)",
             title="Matched asymptotic: tip to body (ε=$ε)",
             size=(900, 500), legend=:topleft)

    ξ_ref = range(0, FIGURE6_XI_MAX, length=200)
    plot!(p, ξ_ref, ε .* collect(ξ_ref), label="εξ (undisturbed cone)",
          color=:gray, linestyle=:dash, linewidth=1, alpha=0.6)

    idx_in = inner_sol.ξ .≤ ξ_match + 5
    plot!(p, inner_sol.ξ[idx_in], inner_sol.S[idx_in],
          label="Inner (nonlinear BVP)", color=:blue, linewidth=2, alpha=0.4)
    plot!(p, outer_lin.ξ, outer_lin.S,
          label="Outer O(ε³) linearised", color=:red, linewidth=1.5,
          alpha=0.4, linestyle=:dash)
    plot!(p, outer_full.ξ, outer_full.S,
          label="Outer (full nonlinear)", color=:orange, linewidth=2,
          alpha=0.5, linestyle=:dashdot)
    plot!(p, ξ_full, S_full, label="Composite (tip → body)",
          color=:black, linewidth=2.5)

    vspan!(p, [blend_lo, blend_hi], label="blend", fillcolor=:yellow, alpha=0.1)
    scatter!(p, [ξ₀], [S₀], label="tip (ξ₀=$(round(ξ₀,digits=2)))",
             color=:red, markersize=5, markerstrokewidth=0)

    xlims!(p, ξ₀ - S₀ - 0.3, 50); ylims!(p, 0, 5.5)
    save_figure(p, "fig6_matched_asymptotic")

    # ── Excess plot: linearised vs reconstructed nonlinear ──
    p2 = plot(xlabel="ξ", ylabel="S(ξ) − εξ",
              title="Linearised O(ε³) vs reconstructed nonlinear outer (ε=$ε)",
              size=(900, 450), legend=:topright)
    hline!(p2, [0], label="", color=:gray, linestyle=:dash, alpha=0.5)

    inner_exc = inner_sol.S .- ε .* inner_sol.ξ
    plot!(p2, inner_sol.ξ[idx_in], inner_exc[idx_in],
          label="Inner", color=:blue, linewidth=2)
    plot!(p2, outer_lin.ξ, outer_lin.S .- ε .* outer_lin.ξ,
          label="Outer O(ε³)", color=:red, linewidth=2, linestyle=:dash)
    plot!(p2, outer_full.ξ, outer_full.S .- ε .* outer_full.ξ,
          label="Outer (full nonlinear)", color=:orange, linewidth=2.5)
    plot!(p2, ξ_all, S_comp .- ε .* ξ_all,
          label="Composite", color=:black, linewidth=2.5)

    vline!(p2, [ξ₀], label="", color=:black, linestyle=:dot, alpha=0.3)
    vspan!(p2, [blend_lo, blend_hi], label="", fillcolor=:yellow, alpha=0.1)
    xlims!(p2, ξ₀ - 0.5, 50)
    save_figure(p2, "fig6_matched_excess")

    (outer_linearised=diagnostic_summary(outer_lin; problem_kind=:outer_linearised),
     outer_full=diagnostic_summary(outer_full; problem_kind=:outer_full))
end

# ── Run all ────────────────────────────────────────────────────────────
function main(args=ARGS)
    if "--metadata-only" in args
        write_figure_metadata()
        return
    end

    println("Generating figures for SlenderConeRecoil...")
    println("Solving inner BVP (3D Newton with axial curvature)...")
    sol = solve_inner_bvp(ε=FIGURE_EPSILON,
                          ξ₀=INNER_XI0_INITIAL,
                          S₀=INNER_S0_INITIAL,
                          Sξξ₀=INNER_SXIXX0_INITIAL,
                          ξ_max=INNER_XI_MAX,
                          newton_iters=INNER_NEWTON_ITERS,
                          newton_tol=INNER_NEWTON_TOL,
                          ode_maxiters=INNER_ODE_MAXITERS)
    if sol.converged
        println("  Newton converged: ξ₀=$(round(sol.ξ₀, digits=3)), S₀=$(round(sol.S₀, digits=4)), residual_norm=$(round(sol.final_residual_norm, sigdigits=4))")
    else
        println("  Newton did not meet tolerance: reason=$(sol.termination_reason), residual_norm=$(round(sol.final_residual_norm, sigdigits=4)), iterations=$(sol.iterations)")
        println("  Continuing with diagnostic solution: ξ₀=$(round(sol.ξ₀, digits=3)), S₀=$(round(sol.S₀, digits=4))")
    end
    println()

    figure1(sol)
    figure2(sol)
    figure3(sol)
    figure4(sol)
    figure5(sol)
    outer_diagnostics = figure6(sol)
    write_figure_metadata(sol=sol,
                          outer_linearised=outer_diagnostics.outer_linearised,
                          outer_full=outer_diagnostics.outer_full)
    println("\nAll figure PDFs, PNGs, and metadata saved to figures/")
end

if abspath(PROGRAM_FILE) == abspath(@__FILE__)
    main(ARGS)
end
